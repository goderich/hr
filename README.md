...
* [`src/smyrf/`](src/smyrf/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/smyrf/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `smyrf` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/smyrf/config.cljs) defaults to `true` in
...
```clj
(ns smyrf.example
  (:require [smyrf.config :as config])
